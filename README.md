...
« Congédier » (en-tête du panneau de discussion, ou « × » sur sa ligne dans « Équipe », après confirmation) fait partir un employé. Piloté : sa réponse en cours s'arrête, ses demandes de validation sont refusées, ses tickets en cours et en attente passent en échec et sa session SDK est fermée ; sa conversation reste sur disque, reprenable par « Reprendre une conversation… ». Terminal : il disparaît seulement du jeu (son ticket en cours échoue), sa session Konsole n'est pas touchée ; il ne revient pas pour cette session, une nouvelle session apparaît normalement.
...
- Commandes slash (employé piloté, ticket commençant par `/`) : `/model <nom>` change son modèle (`set_model`), `/clear` repart sur une nouvelle conversation (`chat_cleared` `{"agent_id"}`, fatigue à 0), les autres commandes de sa session (skills, commandes projet, `/compact`, `/context`…) partent comme prompt ; une commande absente de sa liste → `ticket_rejected`. La liste arrive à son premier ticket : `commands` `{"agent_id", "commands": [{"name", "description", "argumentHint"}]}` (aussi dans le `snapshot`), et sert à l'autocomplétion du panneau (Tab accepte la première).
